...
public class CurrentUserController {
    ...
    public ApiResponse<CurrentUserResponse> updateProfile(@Valid @RequestBody UserProfileUpdateRequest request) {
        var principal = securityPrincipalAccessor.requireCurrentPrincipal();
        return ApiResponse.success(
            currentUserApplicationService.updateProfile(principal.userId(), principal.currentProjectId(), request)
        );
    }
    ...
    public ApiResponse<Void> changePassword(@Valid @RequestBody UserPasswordChangeRequest request) {
        var principal = securityPrincipalAccessor.requireCurrentPrincipal();
        currentUserApplicationService.changePassword(principal.userId(), principal.currentProjectId(), request);
        return ApiResponse.success();
    }
