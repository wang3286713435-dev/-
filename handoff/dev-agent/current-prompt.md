...
7. `handoff/main-agent/m1d-workspace-ia-correction.md`
...
本轮还必须并入一个信息结构纠偏：项目工作台应该呈现为 `项目资产 -> 工程主数据 -> 交付工作中心`。工作中心不是工程主数据的子功能，但必须在项目工作台下，并排在工程主数据之后。用户需要看懂“先定义规则，再按规则交付”。
...
项目工作台或项目内导航也必须更清楚表达三段顺序：

1. `项目资产`：看真实文件、目录、文件详情。
2. `工程主数据`：基于资产线索确认项目结构和交付标准。
3. `交付工作中心`：按已确认标准做补交、审核、整改和预检查。

工程主数据未就绪时，工作中心入口和交付页面必须提示“请先生成 / 确认工程主数据草案”；工程主数据已就绪后，再开放文档交付、图纸交付、审核整改和交付包预检查。
...
- 优化项目工作台信息结构，让 `项目资产 -> 工程主数据 -> 交付工作中心` 的顺序更明确。
- 在工作中心入口和页面上根据工程主数据就绪状态展示准入提示。
...
4. 交付页面流程引导改了什么。
5. 项目工作台信息结构如何体现 `项目资产 -> 工程主数据 -> 交付工作中心`。
6. 工程主数据未就绪时，工作中心如何提示先生成 / 确认工程主数据草案。
7. 缺失项、补交、审核、整改、复审和完整率刷新如何验证。
8. 导出预检查 dry-run 如何保持安全边界。
9. 105/503 与 93/506 抽查结果。
10. 是否触碰后端；如果触碰，说明是否只在 work-center/delivery 范围。
11. 修改文件清单。
12. 自测命令结果。
13. P0 / P1 / P2 列表。
14. 是否建议进入 M1D 测试验收。
