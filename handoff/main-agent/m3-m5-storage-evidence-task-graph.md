...
- 新增全项目对象化覆盖率只读报告：`GET /api/data-steward/storage-objectification-coverage`。
- 当前可解释项目总数：`97`。
- 状态分布：`COMPLETED=1`，`PARTIAL=13`，`NAS_ONLY=2`，`FAILED_NEEDS_GOVERNANCE=0`，`EXCLUDED=81`。
- 平台总文件数：`41214`；已对象化文件：`2994`；NAS_ONLY 文件：`38220`。
- 平台对象化覆盖率：`7.26%`；checksum 覆盖率：`10.72%`。
- 105 / `projectId=503` 为完整样板：`2928 / 2928`，对象化覆盖率 `100.00%`，checksum 覆盖率 `100.00%`，读取策略 `OBJECT_FIRST`，治理项 `0`。
- 非 105 项目状态已按 `PARTIAL / NAS_ONLY / EXCLUDED` 可解释展示，剩余 NAS_ONLY / PARTIAL 项目进入后续批量对象化计划，不伪装成已完成。
- 本批报告查询为 dry-run / 只读口径，未创建迁移任务，未触碰真实 NAS 原文件，禁出字段扫描通过。
...
- [x] M3G-9 已完成，全项目对象化覆盖率报告可查。
- [x] 新上传文件默认对象存储优先写入，并已稳定回归。
- [x] 已对象化文件默认从 NAS 侧 MinIO 读取。
- [x] NAS_ONLY 文件仍可用，但不会被误标为对象化。
- [x] NAS 原项目资料未被移动、删除、重命名、覆盖。
- [x] file-access、预览、下载、交付包清单都不泄露 raw path、bucket、object key。
- [x] M3 系列专项脚本与 M2 关键回归通过。

M3 主 agent 收口结果：

- [x] 2026-06-05 已完成 `M3-CLOSE：对象存储主链路整体收口判断`。
- [x] 收口记录已写入 `handoff/main-agent/m3-overall-closure-report.md`。
- [x] 当前可对外表述为“对象存储主链路完成，105 样板项目完成；非 105 项目仍按 M3X 继续分批对象化”。
...
- [x] M3X：脚本化小批对象化推进
- [x] M3X-Q：全平台对象化后台任务队列与可视化
- [x] M3X-Q-UX1：对象化后台队列极简控制台
- [x] M3X-Q-UX1-F1：旧对象化跑批入口权限收口

M3X-Q 目标：

- 用持久化后台任务队列替代“推一下动一下”的手工脚本推进方式。
- 后端服务运行期间，自动按小批次慢速对象化全平台文件。
- 支持暂停、继续、失败重试、失败原因查看。
- 在 `资产治理 / 对象存储` 页面展示全平台覆盖率、项目进度、当前后台任务和失败项。
- 服务重启后任务状态仍可解释，不丢进度。
- 仍然不移动、不删除、不重命名、不覆盖真实 NAS 原文件。
- 权限边界已通过 `M3X-Q-F1` 收紧：只有超级管理员 `admin` 可以 dry-run、创建、暂停、继续、失败重试或取消全平台对象化后台任务。
- 使用体验已通过 `M3X-Q-UX1` 收敛：默认页面只展示对象化状态、进度、失败项、MinIO 状态和开始 / 暂停 / 继续等核心操作，复杂参数进入高级诊断。
- 旧入口已通过 `M3X-Q-UX1-F1` 收口：旧 `storage-objectification-run:*` 全项目跑批接口同样要求超级管理员，普通用户不可见也不可调用。
...
- M3G-9 已验收通过，M3 已接近整体收口，但 `docs/**` 仍存在旧 M1/M2/8B-0 口径。
- 用户要求先开独立文档收口批次，更新 PRD、设计文档、API 文档维护说明和后续任务图。
- 全项目对象化必须后续单独开批次执行，不混入本次文档收口。
...
1. 输出 M3 整体收口判断。
2. 单独启动 `M3X：全项目对象化独立批次`。
3. M3 收口后再进入 M4 / M5 语义证据与 Hermes 受控问答路线。
