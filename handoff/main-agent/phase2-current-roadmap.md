...
- M3G-1 已收口，NAS 侧 MinIO readiness、全项目对象化盘点和 dry-run 已可用。
- 当前 active 批次为 `M3G-2：105 项目历史文件对象化上传灰度`。
...
- 下一步优先级：
  1. `M3G-2：105 项目历史文件对象化上传灰度`。
  2. `M3G-3：多真实项目分批对象化策略与任务中心增强`。
...
本批只针对 105 / `projectId=503` 执行小批量历史文件对象化。对象化含义是把 NAS 原文件复制一份到 NAS 侧 MinIO，并写入 MySQL 对象台账与 active object version；NAS 原项目资料不移动、不删除、不改名。
