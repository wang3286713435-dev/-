...
当前分支：以实际开发 agent 所在分支为准，建议新批次使用 `codex/m3c-storage-migration-task-center`
...
- 9A 客户交付准备不启动。
- M3A / M3B / M3C-0 / M3C-1 已收口，当前进入 M3C 迁移任务中心。
...
1. `M3C：迁移任务中心与批量策略`（当前）
2. `M3D：真实 NAS 小范围灰度镜像`
3. `M3E：预览与转换产物对象化`
4. `M4：语义证据层`
5. `M5：Hermes 受控证据问答`
