...
## 2026-05-21 路线更新：M2B 收口，下一步进入 UX1 前端体验专项

最新状态：

`M2B：受控 NAS 写操作真实项目灰度试运行与安全开关` 已收口。
...
先完成 M2B Git checkpoint / push，再进入：
...
- UX1 只调整前端路由、菜单、项目工作台、组件布局、样式、交互文案和页面体验。
- UX1 不修改后端业务逻辑、不新增后端接口、不改数据库、不改变权限规则。
...
- 当前为 `绿灯`。
- NAS 写能力仍处于受控灰度阶段，不代表默认开放真实项目写操作。
- 任何永久删除、路径泄露、越权写入、跨项目移动都继续直接判 P0。

## 2026-05-21 路线更新：M1F 收口，进入 M2A 受控 NAS 写操作

最新状态：

`M2A：NAS 受控文件操作安全底座` 已收口。
...
- 当前无 P0 / P1 / P2。
- M2A 专项脚本 `PASS=20 FAIL=0`。
- M1F / M1E / M1D / M1C / Phase2 batch4 回归通过。
- 自动化写操作只在 `/tmp` 隔离测试项目根目录内执行。
- 真实项目页面只做只读检查，未在真实业务 NAS 目录执行上传、移动、删除、重命名等写操作。
- 未发现 raw NAS 路径或 `storage_uri` 泄露。

当前下一步建议：

`M2B：受控 NAS 写操作真实项目灰度试运行与安全开关`

M2B 当前已启动，开发 prompt 已写入：
...
- 当前为 `绿灯`。
- 但 M2A 属于高风险真实文件操作能力，必须使用受控测试目录或测试项目映射先验收。
- 任何永久删除、路径泄露、越权写入、跨项目移动都直接判 P0。

当前交接文件：

- M2A 主 agent 计划：`handoff/main-agent/m2a-controlled-nas-write-plan.md`
- M2A 开发 prompt：`handoff/dev-agent/current-prompt.md`
- M2A 测试 prompt：`handoff/test-agent/current-prompt.md`
...
`M1E：文件管理连续工作体验与后台任务可追踪性收口` 已收口。

测试 agent 验收通过，当前无 P0 / P1 / P2。

M1E 已完成：

- 文件管理按项目记住目录、筛选、分页和最近文件上下文。
- 文件 ID、项目内部 ID、checksum 任务 ID 业务化解释。
- checksum 后台任务在项目内可见、可理解、可重试。
- 继续禁止真实 NAS 写操作、Hermes/G4、8B/8C/9A。

历史下一步建议：

`M1 阶段复盘与 M2A 准备`

现已由 2026-05-21 路线更新取代：M1F 已收口，用户已确认进入 M2A。
