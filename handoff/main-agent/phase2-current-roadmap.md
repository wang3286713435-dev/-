...
二期插入批次 G1、G2、G3 均已正式收口。当前主线切到：

`G4：真实项目交付闭环试运行与问题修补`

G4 目标：

- 用 105 项目跑一条真实项目数字化交付闭环试运行。
- 至少抽查另一个真实 NAS 项目，证明能力不是为 105 写死。
- 验证真实项目资产入口、项目工作台、Hermes Action Center、工程主数据、推荐挂接、文档/图纸交付、审核整改、导出预检查和审计是否能连成可用闭环。
- 只修试运行中暴露的 P0/P1/P2，不扩成新功能大模块。

G4 当前状态：

- G4 规划、开发 prompt、测试 prompt 已写入 handoff。
- 8B、8C、9A 均尚未正式进入。
- G4 通过后，再由用户决定是否恢复 8B / 8C / 9A 或其他后续主线。
- Hermes 后续接入按 `Catalog Layer -> Evidence Layer -> Memory Layer -> Orchestration Layer` 推进。
- G4 只验证并修补当前 catalog-only + 受控平台动作闭环，不进入正文证据、长期记忆或多 Agent 编排实现。
