...
专业字段治理策略：由于当前数字化交付平台中的专业字段尚未形成稳定业务使用场景，V0.1-beta 应趁早把专业编码、中文专业名、中文别名和平台字段映射定义为公司基础标准草案。现有平台 `discipline` 字段只作为现状输入和映射对象，不应反向决定公司专业标准；后续新项目录入、历史文件识别、人工补录和平台字段治理应逐步向本标准收敛。
