...
1. `M1：平台本体稳定期`，先确认平台不依赖 Hermes 也能被普通员工使用。
2. `M2：客户版核心业务补齐期`，补齐数据管家、权限审计、交付包和档案目录。
3. `M3：BIM 引擎前置准备期`，先完成引擎选型和 PoC 标准，再进入真实接入。
4. `M4：客户交付准备期`，在平台核心闭环和 BIM 路线明确后再启动 9A。
